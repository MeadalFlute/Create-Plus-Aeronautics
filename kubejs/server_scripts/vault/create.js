LootJS.lootTables(event => {
    event.create("etc:create_vault", "vault")
    // 基础池
        .createPool(pool => {
            // 资源
            ...
            // 装备
            pool.addEntry(LootEntry.of("create:copper_diving_boots").withWeight(5).enchantWithLevels([15,60]).enchantRandomly().withQuality(1))
            pool.addEntry(LootEntry.of("create:copper_diving_helmet").withWeight(5).enchantWithLevels([15,60]).enchantRandomly().withQuality(1))
            pool.addEntry(LootEntry.of('create:copper_backtank[create:banktank_air=900]').withWeight(5).enchantWithLevels([5,60]).enchantRandomly().withQuality(2))
            pool.addEntry(LootEntry.of("create:netherite_diving_boots").withWeight(2).enchantWithLevels([25,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of("create:netherite_diving_helmet").withWeight(2).enchantWithLevels([25,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of('create:netherite_backtank[create:banktank_air=900]').withWeight(2).enchantWithLevels([5,60]).enchantRandomly().withQuality(2))
            pool.addEntry(LootEntry.of("create:cardboard_sword").withWeight(5).enchantWithLevels([5,60]).enchantRandomly().withQuality(2))
            // 调取整套纸板套——enchant在这里可能有问题
            pool.addEntry(LootEntry.reference("etc:cardboard_suit").enchantWithLevels([15,60]).withWeight(5).withQuality(2))
            
            
            pool.addEntry(LootEntry.of('create_jetpack:jetpack[create:banktank_air=900]').withWeight(2).enchantWithLevels([15,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of('create_jetpack:netherite_jetpack[create:banktank_air=900]').withWeight(1).enchantWithLevels([25,60]).enchantRandomly().withQuality(5))   

            pool.addEntry(LootEntry.of("create:potato_cannon").withWeight(5).enchantWithLevels([15,60]).enchantRandomly().withQuality(2))
            ...
            pool.addEntry(LootEntry.of("create:creative_motor").withWeight(1).setCount(1).withQuality(20))
        ...
        .createPool(pool => {
            // 低概率获得传说级物品
            pool.addEntry(LootEntry.of("create:creative_motor").withWeight(1).setCount(1).withQuality(10).when(condition => {
                condition.randomChance(0.2) // 20%的概率
            }))
            pool.addEntry(LootEntry.of('create_jetpack:jetpack[create:banktank_air=900]').withWeight(2).enchantWithLevels([15,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of('create_jetpack:netherite_jetpack[create:banktank_air=900]').withWeight(1).enchantWithLevels([25,60]).enchantRandomly().withQuality(5))   
            pool.addEntry(LootEntry.of('create:netherite_backtank[create:banktank_air=900]').withWeight(2).enchantWithLevels([5,60]).enchantRandomly().withQuality(2))
            pool.addEntry(LootEntry.of("create:netherite_diving_boots").withWeight(2).enchantWithLevels([25,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of("create:netherite_diving_helmet").withWeight(2).enchantWithLevels([25,60]).enchantRandomly().withQuality(3))
            pool.addEntry(LootEntry.of('minecraft:heavy_core').withWeight(2).setCount(1).withQuality(3))
            pool.rolls(0)   // 基础抽取次数
